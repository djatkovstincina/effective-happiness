...
import { selectSelectedUser, selectUsers } from "../redux/user/userSlice";
...
const BlogPostPage = () => {
    ...
    const postUser = isNewPost ? selectedUser : users.find((user) => user.id === post?.userId);
    ...
    const handleSave = useCallback(async () => {
        setLoading(true);
        try {
            if (isNewPost && postUser) {
                if (!title || !body) {
                    alert('Please fill the title and the body text fields.');
                    setLoading(false);
                    return;
                }
                await dispatch(addBlogPost({ id: Date.now().toString(), title, body, userId: postUser.id })).unwrap();
            } else {
                if (!post?.id) {
        ...
        setLoading(false);
    }, [dispatch, navigate, isNewPost, postUser, title, body, post?.id]);
            ...
            {isEditing ? (
                <>
                    <p className="mb-4 font-semibold">
                        <strong>Author:</strong> {postUser?.first_name} {postUser?.last_name}
                    </p>
